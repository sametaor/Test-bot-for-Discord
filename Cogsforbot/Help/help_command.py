...
class MyHelpCommand(commands.MinimalHelpCommand):
    ...
    async def _help_embed(
        ...
        command_set: Optional[Set[commands.Command]] = None, set_author: bool = False, set_thumbnail = False, colour: nextcord.Colour = None
    ) -> Embed:
        embed = Embed(title=title)
        if description:
            embed.description = description
        if colour == None:
            embed.colour = self.context.author.colour
        if set_thumbnail:
            helpurl = "https://i.imgur.com/1r6Xe0o.png"
            embed.set_thumbnail(url=helpurl)
        if set_author:
...
#Credits to Jonah Lawrence for the tutorial on how to make this paginated help cmd